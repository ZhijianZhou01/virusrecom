...
Take the file ```_Possible recombination event in query_recombinant_conciseness.txt``` generated by using the test data and the above parameters in ```4.1. Unaligned input-sequences``` as an example, the output results are as follows:
