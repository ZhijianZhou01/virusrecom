...
def gap_processing(seq_df,gaps_use):
    ...
    if gaps_use.upper() == "N":
        ...
        seq_pd_clean = seq_df[~seq_df.isin(["-"])].dropna(axis=1,how='any')
...
def lineage_wic_run(query_seq, each_lineage,
    ...
    for (columnName, columnData) in lineage_seq_df.items():
        ...
        if query_seq_count == 1:
            query_nt = query_seq[columnName].iloc[0]
            query_nt_ratio = 1
    ...
    #       + query_lineage_name + " has been completed!" + "\n")
...
def wic_calculation(seq_data,
    ...
    #query_seq = seq_data[seq_data.index.str.contains(query_lineage_name) == True]

    query_seq_list = all_lineage_mapping_dic[query_lineage_name]

    try:
        query_seq = seq_data.loc[query_seq_list]  # more safety

    except Exception as e:
        raise RuntimeError(
            f"Unexpected error while extracting data from lineage '{query_lineage_name}': {e}")

    query_seq_count = query_seq.shape[0]

    if query_seq_count == 0:
        print(">>> Error! The query lineage '" + query_lineage_name + "'"
              + " is not in sequence data." + "\n")
    ...
    columns_name_list = ["Original_Index", "Current_Index"] + ref_lineage_name_list
    ...
    for each_lineage in ref_lineage_name_list:
    
        #lineage_seq_df = seq_data[seq_data.index.str.contains(each_lineage) == True]

        ref_lineage_seq_list = all_lineage_mapping_dic[each_lineage]

        try:
            lineage_seq_df = seq_data.loc[ref_lineage_seq_list]    # more safety

        except Exception as e:
            raise RuntimeError(
                f"Unexpected error while extracting data from lineage '{each_lineage}': {e}")

        lineage_result = p.apply_async(lineage_wic_run,
                                       args=(query_seq, each_lineage, lineage_seq_df,
                                             used_calEnt, query_lineage_name))
...
def wic_compute_engine(seq_file_path,
    ...
    site_ic_csv = (site_dir + "/"+ query_lineage_name + "_site_WIC.csv")
