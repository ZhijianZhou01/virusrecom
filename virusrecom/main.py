...
def parameter():
    ...
    ## new rule of getting lineage information since version 1.4
    parser.add_argument(
        "-map", dest="mapping",
        help="The table of sequence-to-lineage mapping.",
        default="")
...
def starts():
    ...
    print("  Version: 1.4.0 (2025-12-15)")
    ...
    parameter_dic["lineage_mapping"] = myargs.mapping.replace("\\", "/")  # new rule of getting lineage information since version 1.4
    ...
    query_lineage_name = parameter_dic["query_lineage_name"].strip()
    ...
    if parameter_dic["input_wic"] != "":
        ...
        all_lineage_mapping_dic = {}
        all_lineage_list = []

        with open(parameter_dic["lineage_mapping"], "r", encoding="utf-8") as lineage_mapping_file:
            for line in lineage_mapping_file:
                line = line.strip()
                if line != "" and not line.startswith("#"):
                    seq_name, lineage_name = line.split("\t")
                    seq_name = seq_name.strip()
                    lineage_name = lineage_name.strip()

                    if lineage_name not in all_lineage_mapping_dic:
                        all_lineage_mapping_dic[lineage_name] = [seq_name]
                        all_lineage_list.append(lineage_name)

                    else:
                        all_lineage_mapping_dic[lineage_name].append(seq_name)

        if query_lineage_name.upper() != "AUTO":
            ...
            check_label = False
            for lineage_name in all_lineage_list:
                if lineage_name == query_lineage_name:
                    check_label = True

                else:
                    refer_lineage_list.append(lineage_name)

            if not check_label:
                print("Error! The query lineage not in file " + parameter_dic["lineage_mapping"])
                sys.exit()
        ...
        elif query_lineage_name.upper() == "AUTO":

            for each_lineage in all_lineage_list:
                query_lineage_run = each_lineage

                new_refer_lineage_name_list = []

                for lineage in all_lineage_list:
                    if lineage != query_lineage_run:
                        new_refer_lineage_name_list.append(lineage)
